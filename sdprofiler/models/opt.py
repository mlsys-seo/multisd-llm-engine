...
class OPTAttention(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.k_proj = nn.Linear(self.embed_dim, self.q_size, bias=self.enable_bias)
        self.v_proj = nn.Linear(self.embed_dim, self.kv_size, bias=self.enable_bias)
        self.q_proj = nn.Linear(self.embed_dim, self.kv_size, bias=self.enable_bias)
        self.out_proj = nn.Linear(self.q_size, self.embed_dim, bias=self.enable_bias)
...
class OptFlashInferAttention(OPTAttention):
    """
    ...
    def forward(
        ...
        attn_output = attn_output.reshape(num_token, self.embed_dim)
        attn_output = self.out_proj(attn_output)
...
class OPTDecoderLayer(nn.Module):
    def __init__(self, config: OPTConfig, layer_idx: int):
        ...
        self.self_attn = OptFlashInferAttention(config, layer_idx)
