...
class LlamaRMSNorm(nn.Module):
    ...
    def forward(
        ...
        if residual is None:
            ...
            # vllm_ops.fused_add_rms_norm(hidden_states, residual, self.weight, self.variance_epsilon)
...
class LlamaMLP(nn.Module):
    ...
    def combine_gate_up(self):
        ...
        self.gate_up_proj = nn.Linear(hidden_size, intermediate_size + intermediate_size, bias=self.config.mlp_bias, dtype=self.gate_proj.weight.dtype, device=self.gate_proj.weight.device)
...
class LlamaForCausualLMWithSelfSpeculation(LlamaForCausalLM):
    def __init__(self, config):
        super().__init__(config)
        self.config = config
        self.draft_count = 0
        self.devices = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.temp_hs = torch.empty((self.config.num_draft_steps+1)*self.config.max_batch_size, self.config.hidden_size, dtype=self.config.torch_dtype, device=self.devices)
    ...
    def forward(
        ...
        past_key_values: Optional[List[torch.FloatTensor]] = None,
        inputs_embeds: Optional[torch.FloatTensor] = None,
        start_layer_idx: Optional[int] = 0,
        end_layer_idx: Optional[int] = None,
        hierachical: Optional[bool] = False,
    ) -> Union[Tuple, CausalLMOutputWithPast]:
        # print("------------------ LlamaForCausualLMWithSelfSpeculation.forward ------------------")
        # print(f"  start_layer_idx: {start_layer_idx}")
        # print(f"  end_layer_idx: {end_layer_idx}")
        if start_layer_idx == 0 and end_layer_idx is None:
        ...
        elif start_layer_idx == 0 and end_layer_idx is not None:
            outputs = self.forward_early(
                input_ids=input_ids,
                flashinfer_metadata=flashinfer_metadata,
                attention_mask=attention_mask,
                position_ids=position_ids,
        ...
        elif start_layer_idx > 0 and end_layer_idx is None:
            outputs = self.forward_last(
                input_ids=input_ids,
                flashinfer_metadata=flashinfer_metadata,
                attention_mask=attention_mask,
                position_ids=position_ids,
            ...
            assert False, "Invalid start_layer_idx and end_layer_idx"
        return outputs
    ...
    def forward_all(
        ...
        # print(hidden_states[hidden_states.shape[0] // 2])
        return hidden_states
    ...
    def forward_early(
        ...
        target_layers = self.model.layers[start_layer_idx:end_layer_idx+1]
        hidden_states = self.model.embed_tokens(input_ids)
        position_embeddings = self.model.rotary_emb(hidden_states, position_ids)
        for decoder_layer_idx, decoder_layer in enumerate(target_layers):
            hidden_states = decoder_layer.forward_eager(
                hidden_states=hidden_states,
                flashinfer_metadata=flashinfer_metadata,
                past_key_value=past_key_values[decoder_layer_idx],
                position_ids=position_ids,
        ...
        for i in range(hidden_states.shape[0]):
            self.temp_hs[i * (self.config.num_draft_steps+1) + self.draft_count] = hidden_states[i]
        # print(f"draft_count: {self.draft_count}")
        # import pdb; pdb.set_trace()
        # for i in range(hidden_states.shape[0]):
        #     if i not in self.hidden_dict:
        #         self.hidden_dict[i] = []
        #     self.hidden_dict[i].append(hidden_states[i])  # shape: [hidden]
        # self.temp_hidden_states.append(hidden_states)
        ...
        if hierachical and self.draft_count>self.config.num_draft_steps:
            return hidden_states
        
        hidden_states = self.model.norm(hidden_states)
        # self.temp_position_embeddings.append(position_embeddings)
    ...
    def forward_mid(
        ...
        ### DG org
        # import pdb; pdb.set_trace()
        hidden_states = self.temp_hs[:flashinfer_metadata.num_all_tokens]
        # print("flashinfer_metadata.num_all_tokens.item()", flashinfer_metadata.num_all_tokens.item())
        # sorted_keys = sorted(self.hidden_dict.keys())
        # tensor_list = []
        # for k in sorted_keys:
        #     stacked = torch.stack(self.hidden_dict[k], dim=0)
        #     tensor_list.append(stacked)
        # hidden_states = torch.cat(tensor_list, dim=0)
        # import pdb; pdb.set_trace()
        ...
        for decoder_layer_idx, decoder_layer in enumerate(target_layers):
            hidden_states = decoder_layer.forward_eager(
                hidden_states=hidden_states,
                flashinfer_metadata=flashinfer_metadata,
                past_key_value=past_key_values[start_layer_idx+decoder_layer_idx],
                position_ids=position_ids,
                position_embeddings=position_embeddings
            )
        
        # split_hidden_states = torch.split(hidden_states, 1, dim=0)
        # num_keys = len(split_hidden_states) // (self.config.num_draft_steps+1)

        # for i in range(num_keys):
        #     start = i * (self.config.num_draft_steps+1)
        #     end = (i + 1) * (self.config.num_draft_steps+1)
        #     group = split_hidden_states[start:end]  # list of [1, hidden]
        #     self.hidden_dict[i] = [t.squeeze(0) for t in group]
            
        ### DG org    
        self.temp_hs[:flashinfer_metadata.num_all_tokens] = hidden_states
        
        # for i in range(flashinfer_metadata.input_ids_lengths_cpu.size):
        #     start = flashinfer_metadata.input_ids_indptr_cpu[i]
        #     end = flashinfer_metadata.input_ids_indptr_cpu[i + 1]
        #     src = self.temp_hs[start:end-1].clone()
        #     dst_start = i * self.config.num_draft_steps
        #     dst_end = (i + 1) * self.config.num_draft_steps
        #     self.temp_hs[dst_start:dst_end] = src
            # self.temp_hs[i * (self.config.num_draft_steps):(i+1) * (self.config.num_draft_steps)] = self.temp_hs[start:end-1]
        # hidden_states = self.temp_hs[:(i+1) * (self.config.num_draft_steps)]
        # hidden_states = self.temp_hs[:dst_end]
        # hidden_states = hidden_states[:self.config.num_draft_steps]
    ...
    def forward_last(
        ...
        target_layers = self.model.layers[start_layer_idx:]
        ...
        #     try:
        #         # 예외 가능성이 있는 코드
        #         stacked = self.hidden_dict[k][:flashinfer_metadata.input_ids_lengths_cpu[k]]
        #     except Exception as e:
        #         # print(f"[DEBUG] Error at key {k}")
        #         # print(f"  hidden_dict[{k}] length: {len(self.hidden_dict[k])}")
        #         # print(f"  requested length: {flashinfer_metadata.input_ids_lengths_cpu[k]}")
        #         # print(f"  error: {e}")
        #         import pdb; pdb.set_trace()  # 인터랙티브 디버깅 진입
            
        #     tensor_list.append(stacked)
        #     # import pdb; pdb.set_trace()
        # # print(f"tensor_list: {tensor_list}")
        # flat = [t for group in tensor_list for t in group]
        # hidden_states = torch.stack(flat, dim=0) 
        ...
        for i in range(flashinfer_metadata.input_ids_lengths_cpu.size):
            ...
            start = flashinfer_metadata.input_ids_indptr_cpu[i]
            dst_start = i * self.draft_count
            dst_end = dst_start + flashinfer_metadata.input_ids_lengths_cpu[i]
            
            src = self.temp_hs[dst_start:dst_end].clone()
            self.temp_hs[start:end] = src
            # print(flashinfer_metadata)
            # print(f"start: {start}, end: {end}, dst_start: {dst_start}, dst_end: {dst_end}")
            # import pdb; pdb.set_trace()
        hidden_states = self.temp_hs[:end]
        position_embeddings = self.model.rotary_emb(hidden_states, position_ids)
