...
from ..custom_ops import layernorms
...
class Qwen2RotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids):
        ...
        # Force float32 on position calculation (see https://github.com/huggingface/transformers/pull/29285)
        device_type = x.device.type
        device_type = device_type if isinstance(device_type, str) and device_type != "mps" else "cpu"
        ...
        with torch.autocast(device_type=device_type, enabled=False):
            ...
            # position_ids: [num_tokens]
            # freqs: [num_tokens, hidden_size//2]

            freqs = position_ids.float()[:, None] * self.inv_freq.float()[None, :]  
            # freqs: [num_tokens, head_dim//2]  (head_dim = hidden_size / num_heads)
            emb = torch.cat((freqs, freqs), dim=-1)
            cos = emb.cos()  # [num_tokens, head_dim]
            sin = emb.sin()  # [num_tokens, head_dim]

        # Advanced RoPE types may scale cos/sin
        cos = cos * self.attention_scaling
        sin = sin * self.attention_scaling
...
class Qwen2Attention(nn.Module):
    ...
    def forward(
        ...
        with torch.cuda.nvtx.range("Qwen2FlashInferAttention.forward.append_paged_kv_cache"):
            flashinfer.append_paged_kv_cache(
                key_states,
                value_states,
                flashinfer_metadata.batch_indices,
                flashinfer_metadata.positions,
                past_key_value,
                flashinfer_metadata.paged_kv_indices[:flashinfer_metadata.paged_kv_indices_len],
                flashinfer_metadata.paged_kv_indptr,
                flashinfer_metadata.paged_kv_last_page_len,
            )

        with torch.cuda.nvtx.range("Qwen2FlashInferAttention.forward.prefill_wrapper.run"):
            attn_output = self.config.prefill_wrapper.run(
                query_states,
                past_key_value,
            )

        with torch.cuda.nvtx.range("Qwen2FlashInferAttention.forward.reshape"):
