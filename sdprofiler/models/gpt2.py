...
class GPT2FlashinferAttention(GPT2Attention):
    def __init__(self, config, is_cross_attention=False, layer_idx=None):
    ...
    def forward(
        ...
        query_states, key_states, value_states = self.c_attn(hidden_states).split(self.split_size, dim=1)

        shape_q = (*query_states.shape[:-1], -1, self.head_dim)
        shape_kv = (*key_states.shape[:-1], -1, self.head_dim)

        query_states = query_states.view(shape_q).contiguous()
        key_states = key_states.view(shape_kv).contiguous()
        value_states = value_states.view(shape_kv).contiguous()
...
class GPT2Block(nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        self.attn = GPT2FlashinferAttention(config=config, layer_idx=layer_idx)
        self.ln_2 = nn.LayerNorm(hidden_size, eps=config.layer_norm_epsilon)

        if config.add_cross_attention:
            self.crossattention = GPT2FlashinferAttention(config=config, is_cross_attention=True, layer_idx=layer_idx)
